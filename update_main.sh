...
$QEMU_CMD -m $MEMORY -nographic -smp $CPU_CORES -drive file=$my_img,format=qcow2 \
...
while true
do
    ssh -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=1 localhost -p $port "$UPDATE_CMD"
